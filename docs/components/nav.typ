    ...
    while i < len {
      ...
      if "route" in item {
        ...
        html.li(aria-expanded: is-expanded, {
          ...
          if k < i {
            docs.inline(html.button(icon(16, "chevron-right", "Expand")))
...
#let nav-on-this-page-items(items) = docs.inline(html.ul({
  let i = 0
  let len = items.len()
  while i < len {
    let item = items.at(i)
...
#let big-nav-button(
  ...
  class: "nav-button",
  {
    icon
    html.strong(title)
    docs.inline(html.p(description))
  },
)
