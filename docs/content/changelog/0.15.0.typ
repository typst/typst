...
Typst 0.15 makes some adjustments to how ratios passed to these functions are resolved. Previously, a size of `{200%}` passed to @math.lr.size[`lr.size`] or @math.stretch.size[`stretch.size`] would be interpreted relative to a glyph size that was potentially already affected by @math.display[display sizing]. Now, ratios are always resolved relative to the base glyph size. In practice, this means that, to retain the same apperance, you may need to increase the target sizes you've configured for glyphs that are already larger out of the box in display style (e.g. integrals).
