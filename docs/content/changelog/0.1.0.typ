...
- Manual counters now start at zero. Read the "How to step" section @counter[here] for more details
- The @bibliography.style[bibliography styles] `{"author-date"}` and `{"author-title"}` were renamed to `{"chicago-author-date"}` and `{"chicago-author-title"}`
...
- Added support for clipping in @box.clip[boxes] and @block.clip[blocks]
...
- Added support for @int[integers] with base 2, 8, and 16
- Added symbols for double bracket and laplace operator
- The @link function now accepts @label[labels]
...
- Fixed @par.first-line-indent[`first-line-indent`] being not applied when a paragraph starts with styled text
