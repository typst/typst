...
- Added support for multi-column floating @place.scope[placement] and @figure.scope[figures]
- Added support for automatic @par.line[line numbering] (often used in academic papers)
...
- Highly reduced PDF file sizes due to better font subsetting (thanks to #gh("LaurenzV"))
...
= Layout <layout>
- Added support for multi-column floating placement and figures via @place.scope and @figure.scope.  Two-column documents should now prefer `{set page(columns: 2)}` over `{show: column.with(2)}` (see the @guides:page-setup:columns[Page Setup Guide]).
- Added support for automatic @par.line[line numbering] (often used in academic papers)
...
- The height of a `block`, `image`, `rect`, `square`, `ellipse`, or `circle` can now be specified in @fraction[fractional units]
...
- Fixed empty pages appearing when a @reference:context[context] expression wraps whole pages
...
- Fixed @h.weak[weak spacing] not being trimmed at the start and end of lines in a paragraph (only at the start and end of paragraphs)
- Fixed interaction of weak page break and @pagebreak.to
- Fixed compilation output of a single weak page break
- Fixed crash when @pad[padding] by `{100%}`
...
- Added `typm` highlighting mode for math in @raw.lang[raw blocks]
...
- Multiple @text.stylistic-set[stylistic sets] can now be enabled at once
...
- Fixed @smartquote[smart quotes] for Swiss French
...
- Added @math.stretch[`stretch`] function for manually or automatically stretching characters like arrows or parentheses horizontally or vertically
...
- The @math.mat.delim[`mat.delim`], @math.vec.delim[`vec.delim`], and @math.cases.delim[`cases.delim`] parameters now allow any character that is considered a delimiter or "fence" (e.g. |) by Unicode. The `{delim: "||"}` notation is _not_ supported anymore and should be replaced by `{delim: bar.double}` *(Minor breaking change)*
- Added @math.vec.align[`vec.align`] and @math.mat.align[`mat.align`] parameters
- Added @math.underparen[`underparen`], @math.overparen[`overparen`], @math.undershell[`undershell`], and @math.overshell[`overshell`]
...
- Fixed @math.lr.size[`lr.size`] not affecting characters enclosed in @math.mid[`mid`] in some cases
- Fixed resolving of em units in sub- and superscripts
- Fixed bounding box of inline equations when a @text.top-edge[text edge] is set to `{"bounds"}`
...
- Fixed incorrect @here[`here().position()`] when @place was used in a context expression
...
- The `spacing` property of @list.spacing[lists], @enum.spacing[enumerations], and @terms.spacing[term lists] is now also respected for tight lists
...
- Added support for more @numbering[numbering] formats: Devanagari, Eastern Arabic, Bengali, and circled numbers
- Added @heading.hanging-indent[`hanging-indent`] parameter to heading function to tweak the appearance of multi-line headings and improved default appearance of multi-line headings
...
- Added `fill-rule` parameter to `path` and @polygon.fill-rule[`polygon`] functions
- Fixed color mixing and gradients for @color.luma[Luma colors]
...
- Exported SVGs now contain the `data-typst-label` attribute on groups resulting from labelled @box[boxes] and @block[blocks]
- Fixed a bug where some fonts would not print correctly on professional printers
