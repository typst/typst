...
- There is now a distinction between @par[proper paragraphs] and just inline-level content. This is important for future work on accessibility and means that @par.first-line-indent[first line indent] can now be enabled for all paragraphs instead of just consecutive ones.
...
- The `image` function now supports raw @image.format[pixel raster formats] for generating images from within Typst
- Functions that accept file paths now also accept raw @bytes[bytes], for full flexibility
- WebAssembly @plugin[plugins] are more flexible and automatically run multi-threaded
- Fixed a long-standing bug where single-letter strings in math (`[$"a"$]`) would be displayed in italics
- You can now specify which charset should be @text.font[covered] by which font family
- The @pdf.attach[`pdf.embed`] function lets you embed arbitrary files in the exported PDF
...
= Model <model>
- There is now a distinction between @par[proper paragraphs] and just inline-level content *(Breaking change)*
...
- The @outline function was fully reworked to improve its out-of-the-box behavior *(Breaking change)*
  - @outline.entry[Outline entries] are now @block[blocks] and are thus affected by block spacing
  ...
  - The @outline.entry.fill[`fill`] parameter was moved from `outline` to `outline.entry` and can thus be configured through show-set rules
...
- Added @ref.form[`form`] parameter to `ref` function. Setting the form to `{"page"}` will produce a page reference instead of a textual one.
...
- When the @link function wraps around a container like a @block[block], it will now generate only one link for the whole block instead of individual links for all the visible leaf elements. This significantly reduces PDF file sizes when combining `link` and @repeat.
...
- The `image` function now supports raw @image.format[pixel raster formats]. This can be used to generate images from within Typst without the need for encoding in an image exchange format.
...
= Scripting <scripting>
- Functions that accept file paths now also accept raw @bytes[bytes]
...
- Improved WebAssembly @plugin[plugins]
  - The `plugin` type is replaced by a @plugin[`plugin` function] that returns a @module[module] containing normal Typst functions. This module can be used with import syntax. *(Breaking change)*
...
- Values of the @arguments type can now be added with `+` and @reference:scripting:blocks[joined] in curly-braced code blocks
...
- Improved `repr` of @symbol[symbols], @arguments[arguments], and @type[types]
- Duplicate @symbol[symbol] variants and modifiers are now a hard error *(Breaking change)*

= Math <math>
- Fixed a bug where single letter strings in math (`[$"a"$]`) would be displayed in italics
- Math function calls can now have hyphenated named arguments and support @arguments:spreading[argument spreading]
- Better looking accents thanks to support for the `flac` (Flattened Accent Forms) and `dtls` (Dotless Forms) OpenType features
- Added `lcm` @math.op[text operator]
- The @math.bold[`bold`] function now works with ϝ and Ϝ
- The @math.italic[`italic`] function now works with ħ
- Fixed a bug where the extent of a math equation was wrongly affected by internal metadata
- Fixed interaction of @math.lr[`lr`] and @reference:context[context] expressions
- Fixed weak spacing being unconditionally ignored in @math.lr[`lr`]
- Fixed sub/superscripts sometimes being in the wrong position with @math.lr[`lr`]
...
= Text <text>
- Added support for specifying which charset should be @text.font[covered] by which font family
- Added @smallcaps.all[`all`] parameter to `smallcaps` function that also enables small capitals on uppercase letters
- Added basic i18n for Basque and Bulgarian
- @par.justify[Justification] does not affect @raw[raw] blocks anymore
- @text.cjk-latin-spacing[CJK-Latin-spacing] does not affect @raw[raw] text anymore
...
- Fixed lone @cite[citations] in @align not becoming their own paragraph
...
- Added @pdf.attach[`pdf.embed`] function for embedding arbitrary files in the exported PDF
...
= Removals <removals>
- Removed `style` function and `styles` argument of @measure, use a @reference:context[context] expression instead *(Breaking change)*
...
- Removed compatibility behavior where @counter.display worked without @reference:context[context] *(Breaking change)*
