...
Our paper in the previous chapter had a title and an author list. We'll keep the title as document metadata, while our template accepts a list of authors with their affiliations and the paper's abstract. To keep things readable, we'll add those as named arguments. In the end, we want it to work like this:
