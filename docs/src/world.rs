...
use typst::foundations::{
    Binding, Bytes, Content, Context, Datetime, Dict, Duration, IntoValue, Label,
    LocatableSelector, Module, NativeElement, Packed, PathOrStr, Repr, Scope, Selector,
    ShowFn, Str, Target, Value, array, func,
};
...
/// Marks an HTML element as being inline even when it usually wouldn't be
/// classified like that.
///
/// This is a temporary workaround to make the docs possible to express and will
/// be superseded by better native handling.
#[func]
fn inline(elem: Packed<HtmlElem>) -> Content {
    let mut elem = elem;
    elem.inline = Some(true);
    elem.pack()
}
