...
impl Populate for () {
    fn populate(&mut self, _: Spanned<StyleChain>) {}
}
...
/// See the [module level](self) docs for more information.
#[derive(Debug, Clone, Hash)]
pub struct DocumentFormatOptions(EcoVec<FormatOptions>);

impl DocumentFormatOptions {
...
/// Format options that have been configured by the document.
///
/// See the [module level](self) docs for more information.
pub struct FormatOptions(Box<dyn Bounds>);
...
impl<T> From<T> for FormatOptions
where
