...
impl Synthesize for Packed<FigureElem> {
    fn synthesize(
        ...
        let kind = elem.kind.get(styles).unwrap_or_else(|| {
            elem.body
        ...
        let mut caption = elem.caption.get(styles);
...
impl Show for Packed<FigureElem> {
    ...
    fn show(&self, _: &mut Engine, styles: StyleChain) -> SourceResult<Content> {
        ...
        if let Some(caption) = self.caption.get(styles) {
...
impl Refable for Packed<FigureElem> {
    fn supplement(&self) -> Content {
        ...
        match self.supplement.get(StyleChain::default()).as_ref() {
...
impl FigureCaption {
    ...
    fn get_separator(&self, styles: StyleChain) -> Content {
        self.separator.get(styles).unwrap_or_else(|| {
