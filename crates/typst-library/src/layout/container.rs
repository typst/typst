...
pub struct BoxElem {
    ...
    /// position, relative to the total height of the box's contents.
    ///
    /// The other way to adjust it is to simply shift the default baseline
    /// vertically by some amount, specified as a [relative length]($relative).
    /// For example, the value of `{2pt}` will move it up by that exact
    /// [length]($length) (causing the contents to go _down_, as the align
    /// point moves _up_), whereas `{-40%}` will shift the baseline _down_ by
    /// 40% of the inner contents' total height (thus causing the contents to
    /// move _up_).
    ///
    /// Note that both options can be specified at the same time through a
    /// dictionary with keys `at` and `shift`, respectively. For example, by
    /// specifying `{(at: bottom, shift: 10pt)}`, the box's baseline will be
    /// set to the height exactly `{10pt}` above the bottom of its contents.
    ///
    /// ```example
...
cast! {
    ...
    at: Smart<VAlignment> => Self { at: Some(at), shift: Default::default() },
    shift: Rel<Length> => Self { at: Default::default(), shift: Some(shift) },
    mut dict: Dict => {
