...
pub fn grid_to_cellgrid<'a>(
    ...
    let inset = elem.inset.get(styles);
...
pub fn table_to_cellgrid<'a>(
    ...
    let inset = elem.inset.get(styles);
    let align = elem.align.get_ref(styles);
    let columns = elem.columns.get(styles);
    let rows = elem.rows.get(styles);
    let column_gutter = elem.column_gutter.get(styles);
    let row_gutter = elem.row_gutter.get(styles);
    let fill = elem.fill.get_ref(styles);
    let stroke = elem.stroke.resolve(styles);
...
impl ResolvableCell for Packed<TableCell> {
    fn resolve_cell<'a>(
        ...
        let fill = cell.fill.get(styles).unwrap_or_else(|| fill.clone());
