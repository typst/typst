...
impl BibliographyStyle {
    ...
    pub fn default_citation_style(self) -> CitationStyle {
        CitationStyle::BuiltIn(match self {
            ...
            Self::ChicagoNotes => CitationStyleBuiltIn::ChicagoNotes,
...
cast! {
    CitationStyle,
    self => match self {
        Self::BuiltIn(builtin) => builtin.into_value(),
...
fn num_or_str_to_value(num_or_str: &hayagriva::types::NumOrStr) -> Value {
    match num_or_str {
        hayagriva::types::NumOrStr::Number(n) => (*n).into_value(),
        hayagriva::types::NumOrStr::Str(s) => (&**s).into_value(),
    }
}
...
fn range_to_dict<'a: 'b, 'b, T, I: IntoValue>(
    ...
    let citations = preliminary
        ...
        .map(|(citation, cited)| {
            ...
            if style != current {
                ...
                citation_style = match style {
                    CitationStyle::BuiltIn(builtin) => Some(match builtin {
                        ...
                        CitationStyleBuiltIn::ChicagoNotes => {
                            Box::new(style::ChicagoNotes::new())
                        }
            ...
            let content = if let Some(citation_style) = citation_style.as_mut() {
                ...
                if style == CitationStyle::BuiltIn(CitationStyleBuiltIn::ChicagoNotes) {
