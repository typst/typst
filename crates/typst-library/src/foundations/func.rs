...
use crate::foundations::{
    Args, BindingAccess, BindingGuard, Bytes, CastInfo, Content, Context, Dict, Element,
    IntoArgs, PluginFunc, Repr, Scope, Selector, Since, Str, Type, Value,
...
impl Func {
    ...
    #[func(name = "scope", since = "unreleased")]
    pub fn to_scope(&self, engine: &mut Engine) -> HintedStrResult<Dict> {
        ...
        Ok(scope
            ...
            .filter_map(|(name, binding)| {
                ...
                Some((Str::from(name.clone()), val.clone()))
            })
            .collect())
    }
}
