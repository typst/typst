...
use crate::foundations::{
    BindingContext, Datetime, IntoValue, Regex, Repr, SymbolElem, Value, format_str,
};
...
pub fn in_(ctx: impl BindingContext, lhs: Value, rhs: Value) -> HintedStrResult<Value> {
    if let Some(b) = contains(ctx, &lhs, &rhs) {
        Ok(Value::Bool(b))
    } else {
...
pub fn contains(ctx: impl BindingContext, lhs: &Value, rhs: &Value) -> Option<bool> {
    use Value::*;
    match (lhs, rhs) {
        ...
        (Str(a), Module(b)) => {
            Some(b.scope().get(a).is_some_and(|b| b.read(ctx).is_ok()))
        }
