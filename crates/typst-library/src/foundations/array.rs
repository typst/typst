        ...
        let mut are_in_order = move |mut x, mut y| {
            match by.clone() {
                ...
                None => {
                    ...
                    match (key_of(x.clone()), key_of(y.clone())) {
                        (Ok(a), Ok(b)) => match ops::compare(&a, &b) {
                            // Requirement of stable sorting
                            Ok(Ordering::Less | Ordering::Equal) => Ok(true),
                            Ok(Ordering::Greater) => Ok(false),
                            Err(err) => Err(err).at(span),
                        },
