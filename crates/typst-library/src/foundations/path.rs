...
///
/// ```typ
/// // ❌ Error: path would escape the project root
/// #path("../secret.txt")
/// ```
///
...
impl Repr for VirtualPath {
    fn repr(&self) -> ecow::EcoString {
        ...
        eco_format!("path({})", self.get_with_slash().repr())
    }
}
...
impl PathOrStr {
    ...
    pub fn resolve(&self, within: &VirtualPath) -> HintedStrResult<VirtualPath> {
        match self {
            PathOrStr::Path(v) => Ok(v.clone()),
            PathOrStr::Str(v) => match within.parent() {
                Some(parent) => parent.join(v),
                None => within.join(v),
            }
            .map_err(|err| format_path_error(err, within.root(), v)),
        }
    }
