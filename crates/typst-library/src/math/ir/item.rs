...
impl<'a> MathItem<'a> {
    ...
    pub fn styles(&self) -> Option<StyleChain<'a>> {
        match self {
            Self::Component(comp) => Some(comp.props.styles),
            _ => None,
        }
    }
...
#[derive(Debug, Clone)]
pub enum MathKind<'a> {
    // Inline variants (Size <= 56 bytes)
    // GlyphItem is ~56 bytes, setting the baseline.
    Group(GroupItem<'a>),       // 32 bytes -> Inline
    Line(LineItem<'a>),         // 40 bytes -> Inline
    Primes(PrimesItem<'a>),     // 40 bytes -> Inline
    Glyph(GlyphItem),           // 56 bytes -> Inline
    Text(TextItem),             // 16 bytes -> Inline
    External(ExternalItem<'a>), // 8 bytes  -> Inline
    Box(BoxItem<'a>),           // 8 bytes  -> Inline

    // Boxed variants (Size > 56 bytes)
    // Stored as pointers to avoid bloating the enum.
    Radical(&'a RadicalItem<'a>),               // ~96 bytes
    Fenced(&'a FencedItem<'a>),                 // ~120 bytes
    Fraction(&'a FractionItem<'a>),             // ~72 bytes
    SkewedFraction(&'a SkewedFractionItem<'a>), // ~96 bytes
    Table(&'a TableItem<'a>),                   // Large
    Scripts(&'a ScriptsItem<'a>),               // ~200+ bytes
    Accent(&'a AccentItem<'a>),                 // ~96 bytes
    Cancel(&'a CancelItem<'a>),                 // ~80 bytes
}
