...
use typst_library::math::ir::{
    ...
    MultilineItem, Position, RadicalItem, ScriptsItem, TableItem, TextItem,
    resolve_equation,
};
...
fn handle_realized(
    ...
    match &comp.kind {
        ...
        MathKind::Multiline(item) => handle_multiline(item, ctx, styles, props)?,
            ...
            let cell_nodes = row
                ...
                .flat_map(|cell| {
                    ...
                    let nodes: EcoVec<SourceResult<HtmlNode>> = cell
                        ...
                        .map(|sub_col| {
                            Ok(HtmlElement::new(tag::mtd)
                                .with_children(ctx.handle_into_nodes(sub_col, styles)?)
                                // .with_styles(props)
                                .into())
                        })
                        .collect();
