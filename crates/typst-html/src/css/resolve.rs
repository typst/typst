...
impl<'a, 'b> Elem<'a, 'b> {
    fn new(
        ...
        Self { _parent, children: None, attrs, props, selectors }
    }

    fn extend_children(&mut self, new: IdRange<ElemId>) {
        match &mut self.children {
            Some(range) => range.end = new.end,
            None => self.children = Some(new),
        }
    }
}
...
) -> (IdVec<Elem<'a, 'b>>, IdMap<&'a Property, PropRefs>, IdMap<Selector<'b>, SelectorKey>)
{
    ...
    while let Some((parent, children)) = work.pop_front() {
        for node in children {
            ...
            // Add to parent.
            let child_range = IdRange::new(id, elems.next_id());
            elems.get_mut(parent).extend_children(child_range);
...
fn find_group_selectors<'b>(
    bump: &'b Bump,
    temp: &mut Bump,
    elems: &IdVec<Elem<'_, 'b>>,
...
fn identify_group<'b>(
    ...
    let selector_queue = selector_candidates
        ...
        .map(|(id, _, _)| id)
        .collect();
    let mut selector_queue = IdQueue::new(selector_queue, |id| {
