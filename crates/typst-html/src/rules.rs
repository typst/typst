...
use crate::convert::EngineExt;
use crate::{
    FrameElem, HtmlAttr, HtmlAttrs, HtmlElem, HtmlProfile, HtmlTag, attr, css, tag,
};
...
const UNDERLINE_RULE: ShowFn<UnderlineElem> = |elem, _, _| {
    ...
    Ok(HtmlElem::new(tag::span)
        .with_styles(css::Properties::new().with("text-decoration", "underline"), ())
        .with_body(Some(elem.body.clone()))
        .pack())
};
...
const OVERLINE_RULE: ShowFn<OverlineElem> = |elem, _, _| {
    Ok(HtmlElem::new(tag::span)
        .with_styles(css::Properties::new().with("text-decoration", "overline"), ())
        .with_body(Some(elem.body.clone()))
        .pack())
};
...
pub fn html_span_filled(content: Content, color: Color) -> Content {
    ...
    HtmlElem::new(tag::span)
        // TODO: warning sink
        .with_styles(css::Properties::new().with("color", color), ())
...
const BLOCK_RULE: ShowFn<BlockElem> = |elem, engine, styles| {
    let body = match elem.body.get_cloned(styles) {
        // These are only generated by native typst-layout show rules.
        Some(BlockBody::MultiLayouter(_) | BlockBody::SingleLayouter(_)) => {
            bail!(
                elem.span(),
    ...
    match styles.get(HtmlElem::profile) {
        HtmlProfile::Semantic => {
            Ok(HtmlElem::new(tag::div).with_body(body).pack().spanned(elem.span()))
        }
        HtmlProfile::Presentational => styled_container(
            engine,
            elem.pack_ref(),
            body,
            ContainerParams {
...
const BOX_RULE: ShowFn<BoxElem> = |elem, engine, styles| {
    ...
    match styles.get(HtmlElem::profile) {
        HtmlProfile::Semantic => {
            Ok(HtmlElem::new(tag::span)
                // TODO: Maybe even remove this and instead add a `typst-box`
                // class or similar.
...
const IMAGE_RULE: ShowFn<ImageElem> = |elem, engine, styles| {
    ...
    if styles.get(HtmlElem::profile).is_presentational() {
        ...
        match elem.width.get(styles) {
            Smart::Auto => {}
            Smart::Custom(rel) => inline.push("width", rel),
        }

        match elem.height.get(styles) {
            Sizing::Auto => {}
            Sizing::Rel(rel) => inline.push("height", rel),
            Sizing::Fr(_) => engine.ignored(elem.span(), "fractional height"),
        }
    }
...
fn styled_container(
    ...
    if shape.is_some_and(ShapeKind::is_round) {
        ...
        if radius.iter().any(|radius| radius.is_some()) {
            inline.push("border-radius", radius.unwrap_or_default());
        }
    }
    ...
    if clip {
        ...
        inline.push("overflow", "hidden");
    }

    if let Some(fill) = fill {
        inline.push("background", fill);
    }
    ...
    if let Some(css::Padding::NestedElementMargin(margin)) = model.padding {
        ...
        body = Some(
            HtmlElem::new(tag)
                .with_styles(inline, (&mut *engine, elem.span()))
                .with_body(body)
                .pack()
                .spanned(elem.span()),
        );
...
fn container_model_styles(
    ...
    if model.ignored_relative_inset {
        inline.ignored("relative inset");
    }

    match model.width {
        Sizing::Auto => {
            if kind.is_block() {
                inline.push("width", "fit-content");
            }
        }
        Sizing::Rel(rel) => {
            inline.push("width", rel);
        }
        Sizing::Fr(_) => engine.ignored(elem.span(), "fractional width"),
    }

    match model.height {
        Sizing::Auto => {}
        Sizing::Rel(rel) => {
            inline.push("height", rel);
        }
        Sizing::Fr(_) => engine.ignored(elem.span(), "fractional height"),
    }

    if let Some(sizing) = model.box_sizing {
        inline.push("box-sizing", sizing);
    }
