...
impl Content {
    ...
    pub fn field(&self, id: u8) -> StrResult<Value> {
        self.get(id, None)
            .map_err(|e| e.get_message(self.elem().field_name(id).unwrap()))
    }
    ...
    pub fn field_by_name(&self, name: &str) -> StrResult<Value> {
        self.get_by_name(name).map_err(|e| e.get_message(name))
    }
    ...
    pub fn get_message(self, field: &str) -> EcoString {
        match self {
            ...
            FieldAccessError::Internal => {
                eco_format!(
                    "internal error when accessing field {} in content – this is a bug",
                    field.repr()
                )
            }
        }
