...
impl Content {
    ...
    pub fn field(&self, id: u8) -> StrResult<Value> {
        self.get(id, None)
            .map_err(|e| e.get_message(self, self.elem().field_name(id).unwrap()))
    }
    ...
    pub fn field_by_name(&self, name: &str) -> StrResult<Value> {
        self.get_by_name(name).map_err(|e| e.get_message(self, name))
    }
...
impl Content {
    ...
    pub fn at(
        ...
        self.get_by_name(&field)
            ...
            .map_err(|e| e.get_message_no_default(self, &field))
    }
...
impl FieldAccessError {
    ...
    #[cold]
    pub fn get_message(self, content: &Content, field: &str) -> EcoString {
    ...
    #[cold]
    pub fn get_message_no_default(self, content: &Content, field: &str) -> EcoString {
        let mut msg = self.get_message(content, field);
