...
impl LayoutMath for Packed<AccentElem> {
    ...
    fn layout_math(&self, ctx: &mut MathContext, styles: StyleChain) -> SourceResult<()> {
        ...
        let mut base = ctx.layout_into_fragment(self.base(), styles.chain(&cramped))?;

        // Try replace a glyph with its dotless variant.
        if let MathFragment::Glyph(ref mut glyph) = &mut base {
        ...
        let mut glyph = GlyphFragment::new(ctx, styles, *c, self.span());

        // Try replace accent glyph with flattened variant.
