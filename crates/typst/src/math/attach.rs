...
impl LayoutMath for AttachElem {
    #[typst_macros::trace(name = "math.attach", span = self.span())]
...
impl LayoutMath for PrimesElem {
    #[typst_macros::trace(name = "math.primes", span = self.span())]
    fn layout_math(&self, ctx: &mut MathContext) -> SourceResult<()> {
        match *self.count() {
            count @ 1..=4 => {
...
impl LayoutMath for ScriptsElem {
    #[typst_macros::trace(name = "math.scripts", span = self.span())]
    fn layout_math(&self, ctx: &mut MathContext) -> SourceResult<()> {
...
impl LayoutMath for LimitsElem {
    #[typst_macros::trace(name = "math.limits", span = self.span())]
    fn layout_math(&self, ctx: &mut MathContext) -> SourceResult<()> {
        let mut fragment = ctx.layout_fragment(self.body())?;
