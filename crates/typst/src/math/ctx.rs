...
impl<'a, 'b, 'v> MathContext<'a, 'b, 'v> {
    ...
    pub fn layout_math_variable(
        ...
        while let Some(grapheme) = graphemes.next() {
            // Determine the next text item to layout. This is heavily coupled with the
            // previous layout organization and should likely be refactored when things
            // are updated.
    ...
    /// Returns a string of of ascii digits plus dots if there is a run of two or more
    /// characters. Runs have at least one ascii digit and at most one dot.
    ///
    /// This is meant to match how the lexer parses numbers in math into single MathText
    /// elements.
    fn try_number_run(
        grapheme: &str,
        ...
        if number_run.len() == 1 {
            // Note: This handles edge cases like "." and "..". Be careful if changing.
            None
        } else {
            Some(number_run)
        }
    ...
    fn try_glyph(
        ...
        if chars.next().is_some() {
            // Note: I'm Not certain that this is proper for all languages. Don't have
            // enough unicode knowledge.
