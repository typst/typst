...
pub struct CancelElem {
    ...
    /// - If given a function `angle => angle`, the line is rotated, w.r.t
    ///   the y-axis, by the angle returned by that function. The function
    ///   receives the default angle as its input.
    ///
    /// ```example
    /// >>> #set page(width: 140pt)
    /// $ cancel(Pi)
