...
impl HeadingNode {
    fn leaf(element: Content) -> Self {
        HeadingNode {
            level: element.expect_field::<NonZeroUsize>(0),
            // 'bookmarked' set to 'auto' falls back to the value of 'outlined'.
            bookmarked: element
                .expect_field::<Smart<bool>>(2)
                .unwrap_or_else(|| element.expect_field::<bool>(1)),
            element,
            children: Vec::new(),
        }
    }
...
fn write_outline_item(
    ...
    let body = node.element.expect_field::<Content>(3);
