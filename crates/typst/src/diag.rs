...
pub trait At<T> {
    ...
    /// Add the span informaton only if the result is an error.
    fn at_err(self, span: impl Fn() -> Span) -> SourceResult<T>;
}
...
impl<T> At<T> for Result<T, HintedString> {
    ...
    fn at_err(self, span: impl FnOnce() -> Span) -> SourceResult<T> {
        self.map_err(|diags| {
            eco_vec![
