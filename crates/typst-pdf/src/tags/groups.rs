...
use typst_library::layout::{FrameParent, GridCell};
...
impl Groups {
    ...
    pub fn breakable(&self, kind: &GroupKind) -> BreakOpportunity {
        ...
        match kind {
            ...
            GroupKind::TextAttr(_) => Always(BreakPriority::Artifact),
...
impl Groups {
    ...
    fn check_ancestor(&self, parent: GroupId, child: GroupId) -> bool {
        ...
        // Logical children that are `LogicalChildKind::Insert` have their
        // parent set to the the original location they appeared in the tree,
        // but will be inserted into the correct logical parent.
        if matches!(group.kind, GroupKind::LogicalChild(LogicalChildKind::Insert, _)) {
            return true;
        }
...
pub enum GroupKind {
    ...
    LogicalChild(LogicalChildKind, GroupId),
...
impl From<FrameParent> for LogicalChildKind {
    fn from(value: FrameParent) -> Self {
        match value {
            FrameParent::Insert(_) => LogicalChildKind::Insert,
            FrameParent::Inherit(_) => LogicalChildKind::Inherit,
        }
    }
}
