...
impl LastHeading {
    const NONE: Self = Self { level: 0, span: Span::detached() };
}
    ...
    rs.with_flatten(flatten, |rs| -> SourceResult<()> {
        ...
        if group.kind.is_artifact() {
            for child in group.nodes().iter() {
                resolve_artifact_node(rs, lang, bbox, child);
            }
        } else {
...
fn build_group_tag(rs: &mut Resolver, group: &Group) -> SourceResult<Option<TagKind>> {
    let tag = match &group.kind {
        ...
        GroupKind::CodeBlock(_) => {
            Tag::Code.with_placement(Some(Placement::Block)).into()
        }
        GroupKind::CodeBlockLine(_) => Tag::Span.into(),
    ...
    // Check that no heading levels we're skipped.
    if let TagKind::Hn(tag) = &tag {
        let prev_level = rs.last_heading.level;
        let next_level = tag.level().get();
        let span = to_span(tag.as_any().location);
        if rs.options.is_pdf_ua() && next_level.saturating_sub(prev_level) > 1 {
            let validator = rs.options.standards.config.validator().as_str();
            if rs.last_heading.level == 0 {
        ...
        rs.last_heading.level = next_level;
        rs.last_heading.span = span;
    }
