...
impl TableCtx {
    ...
    pub fn insert(&mut self, cell: &TableCell, nodes: Vec<TagNode>) {
        let x = cell.x.get(StyleChain::default()).unwrap_or_else(|| unreachable!());
        let y = cell.y.get(StyleChain::default()).unwrap_or_else(|| unreachable!());
    ...
    pub fn build_table(mut self, mut nodes: Vec<TagNode>) -> TagNode {
        ...
        TagNode::group(Tag::Table.with_summary(self.summary), nodes)
    }
...
mod tests {
    ...
    fn th<const SIZE: usize>(
        ...
        TagNode::group(
            Tag::TH(scope).with_id(Some(id)).with_headers(Some(ids)),
            Vec::new(),
        )
    }
    ...
    fn td<const SIZE: usize>(headers: [(u32, u32); SIZE]) -> TagNode {
        let ids = headers.map(|(x, y)| table_cell_id(TableId(324), x, y)).into_iter();
        TagNode::group(Tag::TD.with_headers(Some(ids)), Vec::new())
    }
