...
pub fn format() -> Format {
    Format::new::<Pdf>().with_rules(register)
}
...
///
/// When using PDF files as @image[images] in your document, the export PDF version must equal or exceed the image file versions.
///
/// == PDF/UA <pdf-ua>
/// Typst supports writing PDF/UA-conformant files. PDF/UA files are designed for _@guides:accessibility:basics[Universal Access]._ When you choose this PDF standard, Typst will run additional checks when exporting your document. These checks will make sure that you are following accessibility best practices. For example, it will make sure that all your images come with alternative descriptions.
///
...
///
/// - If your document is written in a different language than English, make sure @text.lang[set the text language] before any content.
/// - Make sure to use Typst's semantic elements (like @heading[headings], @figure[figures], and @list[lists]) when appropriate instead of defining custom constructs. This lets Typst know (and export) the role a construct plays in the document. See @guides:accessibility[the Accessibility guide] for more details.
...
///
/// Typst currently only supports part one (PDF/UA-1) which is based on PDF 1.7 (2006). When exporting to PDF/UA-1, be aware that you will need to manually provide @math:accessibility[alternative descriptions of mathematics] in natural language.
///
...
///
/// - *PDF/A-2a:* This is the _accessible_ conformance level that builds on the Unicode-mappable level A-2u. This conformance level also adds two requirements: Your file must be an accessible _Tagged PDF_ file. Typst automatically adds tags to help you reach this conformance level. Also pay attention to the Accessibility Sections throughout the reference and the @guides:accessibility[Accessibility Guide] when targeting this conformance level. Finally, PDF/A-2a forbids you from using code points in the #link("https://en.wikipedia.org/wiki/Private_Use_Areas")[Unicode Private Use area]. If you want to build an accessible file, also consider additionally targeting PDF/UA-1, which enables more automatic accessibility checks.
///
/// - *PDF/A-3b:* The _basic_ conformance level of ISO 19005-3. This version of PDF/A is based on PDF 1.7 (2006) and results in archivable PDF files that can contain arbitrary other related files as @pdf.attach[attachments]. The only difference between it and PDF/A-2b is the capability to attach non-PDF/A-conformant files.
///
...
///
/// - *PDF/A-4f:* The _embedded files_ conformance level that builds on the basic level A-4. Files conforming to this level can contain arbitrary other related files as @pdf.attach[attachments], just as files conforming to part 3 of ISO 19005. The only difference between it and PDF/A-4 is the capability to attach non-PDF/A-conformant files.
///
...
///
/// + *If not, does my file need @pdf.attach[attachments]?* If so, you must choose part three (PDF/A-3) or the embedded files level of part four (PDF/A-4f).
///
/// + *Does your file need to use features introduced in PDF 2.0?* Currently, use of PDF 2.0 features in Typst is limited to minor improvements in accessibility, e.g. for the @title[title element]. If you can do without these improvements, maximize compatibility by choosing part two (when you don't need attachments) or part three (when you do need attachments). If you rely on PDF 2.0 features, use part four.
///
...
///
/// This module contains some functions without a final API. They are designed to enhance accessibility for documents with complex tables. This includes @[`table-summary`], @pdf.header-cell[`header-cell`], and @pdf.data-cell[`data-cell`]. All of these functions will be removed in a future Typst release, either through integration into table functions or through full removal. You can enable these functions by passing `--features a11y-extras` or setting the `TYPST_FEATURES` environment variable to `a11y-extras`. In the web app, these features are not available at this time.
