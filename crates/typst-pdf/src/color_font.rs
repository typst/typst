...
use crate::{AllocRefs, BuildContent, Resources, WriteStep};
        ...
        context.resources.write(&mut |resources: &Resources| {
            ...
            for (font, color_font) in &color_fonts.map {
                ...
                for font_index in 0..(color_font.glyphs.len() / 256) + 1 {
                    ...
                    pdf_font.pair(Name(b"Resources"), color_fonts.resources_ref.unwrap()); // TODO: find a way to not unwrap here?
                    ...
                    for (gid, instructions_ref) in
                        glyphs_to_instructions.iter().enumerate()
                    {
                        char_procs.pair(
