...
impl<'a, 'b> Distributor<'a, 'b, '_, '_, '_> {
    ...
    fn multi(&mut self, multi: &'b MultiChild<'a>) -> FlowResult<()> {
        ...
        let (frame, spill) =
            multi.layout(self.composer.engine, self.widen_regions(pod, wide))?;
        if frame.is_empty()
    ...
    fn multi_spill(&mut self, spill: MultiSpill<'a, 'b>) -> FlowResult<()> {
        ...
        let (frame, spill) =
            spill.layout(self.composer.engine, self.widen_regions(pod, wide))?;
    ...
    fn finalize(
        ...
        for item in &self.items {
            if let Item::Fr(v, _, child, _) = item {
                frs += *v;
