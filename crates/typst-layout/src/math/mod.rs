...
pub fn layout_equation_inline(
    ...
    let mut items = if !run.is_multiline() {
        ...
        vec![InlineItem::Frame(ctx.layout_into_fragment(&run)?.into_frame())]
    };
...
impl<'a, 'v, 'e> MathContext<'a, 'v, 'e> {
    ...
    fn layout_into_fragments(
        ...
        self.layout_into_self(run, run.styles())?;
        Ok(std::mem::replace(&mut self.fragments, prev))
    }

    /// Layout the given element and return the resulting [`MathFragment`]s.
    fn layout_into_fragment(&mut self, run: &MathRun) -> SourceResult<MathFragment> {
        let fragments = self.layout_into_fragments(run)?;
        if fragments.len() == 1 {
        ...
        Ok(FrameFragment::new(&props, frame).with_text_like(text_like).into())
    }
