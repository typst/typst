...
use typst_library::math::{MathRun, *};
...
use crate::math::attach::{layout_primes, layout_scripts};
use crate::math::cancel::layout_cancel;
use crate::math::frac::{layout_fraction, layout_skewed_fraction};
use crate::math::lr::layout_fenced;
use crate::math::mat::layout_table;
use crate::math::root::layout_radical;
use crate::math::run::{MathFragmentsExt, MathRunFrameBuilder};
use crate::math::text::{layout_glyph, layout_text};
use crate::math::underover::layout_line;

use self::fragment::{FrameFragment, GlyphFragment, MathFragment, stretch_axes};
use self::shared::*;
...
pub fn layout_equation_inline(
    ...
    let fragments = ctx.layout_into_fragments(&run, styles)?;

    let mut items = if fragments.row_count() == 1 {
...
impl<'a, 'v, 'e> MathContext<'a, 'v, 'e> {
    ...
    /// Layout the given element and return the resulting [`MathFragment`]s.
    fn layout_into_fragment(&mut self, run: &MathRun) -> SourceResult<MathFragment> {
        Ok(self
            .layout_into_fragments(run, run.styles())?
            .into_fragment(run.styles()))
    }
...
fn layout_realized(
    ...
    match &comp.kind {
        MathKind::Box(item) => layout_box(item, ctx, props)?,
        MathKind::External(item) => layout_external(item, ctx, props)?,
        MathKind::Glyph(item) => layout_glyph(item, ctx, props)?,
        MathKind::Cancel(item) => layout_cancel(item, ctx, props)?,
        MathKind::Radical(item) => layout_radical(item, ctx, props)?,
        MathKind::Line(item) => layout_line(item, ctx, props)?,
        MathKind::Accent(item) => layout_accent(item, ctx, props)?,
        MathKind::Scripts(item) => layout_scripts(item, ctx, props)?,
        MathKind::Primes(item) => layout_primes(item, ctx, props)?,
        MathKind::Table(item) => layout_table(item, ctx, props)?,
        MathKind::Fraction(item) => layout_fraction(item, ctx, props)?,
        MathKind::SkewedFraction(item) => layout_skewed_fraction(item, ctx, props)?,
        MathKind::Text(item) => layout_text(item, ctx, props)?,
        MathKind::Fenced(item) => layout_fenced(item, ctx, props)?,
        MathKind::Group(item) => {
            let fragment = ctx.layout_into_fragment(&item.items)?;
...
fn layout_external(
    ...
    if !frame.has_baseline() {
        let axis = ctx.font().math().axis_height.resolve(props.styles);
