...
pub fn layout_radical(
    ...
    let radicand = {
        ...
        let radicand = ctx.layout_into_fragment(&item.radicand)?;
        if multiline {
            // Align the frame center line with the math axis.
            let (font, size) = radicand.font(ctx, item.radicand.styles());
    ...
    let (font, size) = sqrt.font(ctx, item.sqrt.styles());
    ...
    let gap = match item.sqrt.styles().get(EquationElem::size) {
    ...
    let line = FrameItem::Shape(
        Geometry::Line(Point::with_x(radicand.width())).stroked(FixedStroke::from_pair(
            sqrt.fill().unwrap_or_else(|| {
                item.sqrt.styles().get_ref(TextElem::fill).as_decoration()
            }),
            thickness,
        )),
        props.span,
