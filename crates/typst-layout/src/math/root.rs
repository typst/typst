...
#[typst_macros::time(name = "math.root", span = props.span)]
pub fn layout_radical(
    ...
    // Layout radicand.
    let radicand = {
        let fragments = ctx.layout_into_fragments(&item.radicand, props.styles)?;
        let multiline = fragments.is_multiline();
        let radicand = fragments.into_fragment(props.styles);
        if multiline {
            // Align the frame center line with the math axis.
            let (font, size) = radicand.font(ctx, props.styles);
    ...
    // Layout root symbol.
    let mut sqrt = ctx.layout_into_fragment(&item.sqrt)?;

    let (font, size) = sqrt.font(ctx, props.styles);
    ...
    let gap = match props.styles.get(EquationElem::size) {
    ...
    let line = FrameItem::Shape(
        Geometry::Line(Point::with_x(radicand.width())).stroked(FixedStroke::from_pair(
            sqrt.fill()
                .unwrap_or_else(|| props.styles.get_ref(TextElem::fill).as_decoration()),
            thickness,
        )),
        props.span,
    );
    ...
    let index = item
        .index
        .as_ref()
        .map(|index| ctx.layout_into_fragment(index))
        .transpose()?
