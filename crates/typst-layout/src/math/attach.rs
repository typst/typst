...
pub fn layout_attach(
    ...
    let tl = elem.tl.get(sup_style_chain);
    let tr = elem.tr.get(sup_style_chain);
    let primed = tr.as_ref().is_some_and(|content| content.is::<PrimesElem>());
    let t = elem.t.get(sup_style_chain);

    let sub_style = style_for_subscript(styles);
    let sub_style_chain = styles.chain(&sub_style);
    let bl = elem.bl.get(sub_style_chain);
    let br = elem.br.get(sub_style_chain);
    let b = elem.b.get(sub_style_chain);
