...
/// Layout a [`TableItem`].
#[typst_macros::time(name = "math.vec", span = props.span)]
#[typst_macros::time(name = "math.cases", span = props.span)]
#[typst_macros::time(name = "math.mat", span = props.span)]
pub fn layout_table(
    item: &TableItem,
    ctx: &mut MathContext,
    ...
    if ncols == 0 || nrows == 0 {
        ctx.push(FrameFragment::new(props, Frame::soft(Size::zero())));
    ...
    let default_stroke_thickness = DEFAULT_STROKE_THICKNESS.resolve(props.styles);
    ...
    let denom_style = style_for_denominator(props.styles);
    ...
    let paren = GlyphFragment::new_char(
        ctx,
        props.styles.chain(&denom_style),
        '(',
        Span::detached(),
    )
    .unwrap();
    ...
    for (column, col) in columns.iter().zip(&mut cols) {
        for (cell, (ascent, descent)) in column.iter().zip(&mut heights) {
            let cell = ctx.layout_into_fragments(cell, props.styles)?;
