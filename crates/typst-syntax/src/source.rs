...
use crate::{
    FileId, LinkedNode, PreferredCompilerVersion, RootedPath, Span, SpanNumber, SubRange,
    SyntaxNode, VirtualPath, VirtualRoot, parse,
};
...
mod test {
    use super::Source;
    use crate::{LinkedNode, PreferredCompilerVersion, Side, Span, SubRange};

    #[test]
    fn test_source_sub_ranges() {
        let text = "= head <label>";
        let source = Source::detached(text, PreferredCompilerVersion::current());
