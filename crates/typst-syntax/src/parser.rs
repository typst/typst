...
/// Eat a `SyntaxKind::Dots` kind and replace its node with two leaf nodes of
/// kind `SyntaxKind::Text` with text `.`.
fn math_undots(p: &mut Parser) {
    p.eat();
    p.nodes.pop();
    let node = SyntaxNode::leaf(SyntaxKind::Text, '.');
