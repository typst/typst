...
pub(crate) enum ModeAfter {
    /// The mode is known!
    Known(SyntaxMode),
    /// We treat comments as not producing any syntax mode.
    Comment,
...
impl SyntaxKind {
    ...
    pub(crate) fn mode_after(self) -> ModeAfter {
        ...
        match self {
            ...
            Self::RawLang => Parent,    // code/markup: Raw
            Self::RawDelim => Parent,   // code/markup: Raw
            Self::RawTrimmed => Parent, // code/markup: Raw
...
mod test {
    ...
    fn test_mode_after() {
        ...
        test_mode("```l r```", [0, 3, 4, 5, 6], Some(Markup));
        ...
        test_mode("#`r`", [1, 2, 3], Some(Code));
        test_mode("#```l r```", [1, 4, 5, 6, 7], Some(Code));
