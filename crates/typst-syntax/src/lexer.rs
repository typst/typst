    ...
    pub fn next(&mut self) -> (SyntaxKind, SyntaxNode) {
        ...
        let kind = match self.s.eat() {
            ...
            Some(c) => match self.mode {
                ...
                LexMode::MathArg => match self.math_arg(start, c) {
                    (kind, None) => kind,
                    (kind, Some(node)) => return (kind, node),
                },
...
/// Math argument.
/// Identical to Math, but we lex commas, semicolons, and right parens so we
/// know when to stop.
impl Lexer<'_> {
    fn math_arg(&mut self, start: usize, c: char) -> (SyntaxKind, Option<SyntaxNode>) {
        match c {
...
impl Lexer<'_> {
    ...
    fn is_math_spread_arg(&mut self) -> bool {
        ...
        // Check that neither non-whitespace nor dot follows the spread syntax.
        // A dot clashes with the "..." math shorthand.
        if let Some(c) = self.s.peek() {
