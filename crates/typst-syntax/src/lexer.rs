...
impl Lexer<'_> {
    ...
    ///     - The opening delimiter's line does not contribute to the dedent,
    ///       but the closing delimiter's line does (even if that line is
    ///       entirely whitespace up to the delimiter).
    ...
    /// - Otherwise it's text is kept like an inner line. However, if the last
