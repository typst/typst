...
use typst_utils::{Intern, Interner};
...
/// A path in a virtual file system root.
///
/// Roots include the project and packages.
#[derive(Clone, Eq, PartialEq, Hash)]
pub struct VirtualPath {
    /// The kind of root in which the path resides. How to resolve this is up to
    /// the `World` implementor.
    root: VirtualRoot,
    /// The path within the `root`.
    segments: Segments,
}

impl VirtualPath {
    /// Creates a new virtual path.
    pub fn new(root: VirtualRoot, path: impl AsRef<str>) -> Result<Self, PathError> {
    ...
    /// Returns the root relative to which the path is to be interpreted.
    pub fn root(&self) -> &VirtualRoot {
        &self.root
    }

    /// Returns the path within the root with a leading slash.
    pub fn get_with_slash(&self) -> &str {
        self.segments.get_with_slash()
    }

    /// Returns the path within the root without a leading slash.
    pub fn get_without_slash(&self) -> &str {
        self.segments.get_without_slash()
    }

    /// Returns the file name portion of the path.
    pub fn file_name(&self) -> Option<&str> {
        self.segments.last().map(Segment::get)
    }
    ...
    pub fn with_extension(&self, ext: &str) -> Self {
        ...
        Self { root: self.root.clone(), segments }
    }
    ...
    pub fn parent(&self) -> Option<Self> {
        ...
        Some(Self { root: self.root.clone(), segments })
    }
    ...
    pub fn join(&self, path: &str) -> Result<Self, PathError> {
        ...
        Ok(Self { root: self.root.clone(), segments })
    }
    ...
    pub fn relative_from(&self, base: &Self) -> Option<EcoString> {
        // Both paths must be in the same root for this to even be possible.
        ...
        let mut ita = self.segments.iter();
        let mut itb = base.segments.iter();
...
impl VirtualPath {
    ...
    pub fn within_root(path: &Path, root: &Path) -> Option<Self> {
        Self::virtualize(VirtualRoot::Project, root, path).ok()
    }

    /// The absolute and normalized path to the file _within_ the project or
    /// package.
    #[deprecated = "just omit the method call"]
    pub fn vpath(&self) -> &Self {
        self
    }
...
impl Debug for VirtualPath {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        let path = self.get_with_slash();
        match self.root() {
            VirtualRoot::Project => write!(f, "{path}"),
            VirtualRoot::Package(package) => write!(f, "{package:?}{path}"),
        }
    }
}

impl Intern for VirtualPath {
    type Index = NonZeroU16;

    const INTERNER: &'static Interner<Self> = &INTERNER;
}

static INTERNER: Interner<VirtualPath> = Interner::new();
...
mod tests {
    ...
    fn path(p: &str) -> VirtualPath {
        VirtualPath::new(VirtualRoot::Project, p).unwrap()
    }
    ...
    fn test_new() {
        ...
        fn test(path: &str, expected: Result<&str, PathError>) {
            let path = VirtualPath::new(VirtualRoot::Project, path);
            assert_eq!(
