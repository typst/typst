...
impl<'a> AstNode<'a> for Expr<'a> {
    fn from_untyped(node: &'a SyntaxNode) -> Option<Self> {
        Some(match node.kind() {
            SyntaxKind::Space => return Option::None, // Skipped unless using `cast_with_space`.
            SyntaxKind::Linebreak => Self::Linebreak(Linebreak(node)),
            SyntaxKind::Parbreak => Self::Parbreak(Parbreak(node)),
            SyntaxKind::Text => Self::Text(Text(node)),
            SyntaxKind::Escape => Self::Escape(Escape(node)),
            SyntaxKind::Shorthand => Self::Shorthand(Shorthand(node)),
            SyntaxKind::SmartQuote => Self::SmartQuote(SmartQuote(node)),
            SyntaxKind::Strong => Self::Strong(Strong(node)),
            SyntaxKind::Emph => Self::Emph(Emph(node)),
            SyntaxKind::Raw => Self::Raw(Raw(node)),
            SyntaxKind::Link => Self::Link(Link(node)),
            SyntaxKind::Label => Self::Label(Label(node)),
            SyntaxKind::Ref => Self::Ref(Ref(node)),
            SyntaxKind::Heading => Self::Heading(Heading(node)),
            SyntaxKind::ListItem => Self::ListItem(ListItem(node)),
            SyntaxKind::EnumItem => Self::EnumItem(EnumItem(node)),
            SyntaxKind::TermItem => Self::TermItem(TermItem(node)),
            SyntaxKind::Equation => Self::Equation(Equation(node)),
            SyntaxKind::Math => Self::Math(Math(node)),
            SyntaxKind::MathText => Self::MathText(MathText(node)),
            SyntaxKind::MathIdent => Self::MathIdent(MathIdent(node)),
            SyntaxKind::MathShorthand => Self::MathShorthand(MathShorthand(node)),
            SyntaxKind::MathAlignPoint => Self::MathAlignPoint(MathAlignPoint(node)),
            SyntaxKind::MathDelimited => Self::MathDelimited(MathDelimited(node)),
            SyntaxKind::MathAttach => Self::MathAttach(MathAttach(node)),
            SyntaxKind::MathPrimes => Self::MathPrimes(MathPrimes(node)),
            SyntaxKind::MathFrac => Self::MathFrac(MathFrac(node)),
            SyntaxKind::MathRoot => Self::MathRoot(MathRoot(node)),
            SyntaxKind::Ident => Self::Ident(Ident(node)),
            SyntaxKind::None => Self::None(None(node)),
            SyntaxKind::Auto => Self::Auto(Auto(node)),
            SyntaxKind::Bool => Self::Bool(Bool(node)),
            SyntaxKind::Int => Self::Int(Int(node)),
            SyntaxKind::Float => Self::Float(Float(node)),
            SyntaxKind::Numeric => Self::Numeric(Numeric(node)),
            SyntaxKind::Str => Self::Str(Str(node)),
            SyntaxKind::CodeBlock => Self::CodeBlock(CodeBlock(node)),
            SyntaxKind::ContentBlock => Self::ContentBlock(ContentBlock(node)),
            SyntaxKind::Parenthesized => Self::Parenthesized(Parenthesized(node)),
            SyntaxKind::Array => Self::Array(Array(node)),
            SyntaxKind::Dict => Self::Dict(Dict(node)),
            SyntaxKind::Unary => Self::Unary(Unary(node)),
            SyntaxKind::Binary => Self::Binary(Binary(node)),
            ...
            SyntaxKind::DestructAssignment => {
                Self::DestructAssignment(DestructAssignment(node))
            }
            SyntaxKind::SetRule => Self::SetRule(SetRule(node)),
            SyntaxKind::ShowRule => Self::ShowRule(ShowRule(node)),
            SyntaxKind::Contextual => Self::Contextual(Contextual(node)),
            SyntaxKind::Conditional => Self::Conditional(Conditional(node)),
            SyntaxKind::WhileLoop => Self::WhileLoop(WhileLoop(node)),
            SyntaxKind::ForLoop => Self::ForLoop(ForLoop(node)),
            SyntaxKind::ModuleImport => Self::ModuleImport(ModuleImport(node)),
            SyntaxKind::ModuleInclude => Self::ModuleInclude(ModuleInclude(node)),
            SyntaxKind::LoopBreak => Self::LoopBreak(LoopBreak(node)),
            SyntaxKind::LoopContinue => Self::LoopContinue(LoopContinue(node)),
            SyntaxKind::FuncReturn => Self::FuncReturn(FuncReturn(node)),
