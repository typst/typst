...
impl LinkedNode<'_> {
    ...
    pub fn prev_sibling(&self) -> Option<Self> {
        ...
        children.iter = children.iter.as_slice()[..self.index].iter();
    ...
    pub fn next_sibling(&self) -> Option<Self> {
        let mut children = self.parent()?.children();
        children.iter = children.iter.as_slice()[self.index + 1..].iter();
...
pub struct LinkedChildren<'a> {
    ...
    iter: std::slice::Iter<'a, SyntaxNode>,
    index: usize,
    front: usize,
    back: usize,
}
...
impl DoubleEndedIterator for LinkedChildren<'_> {
    fn next_back(&mut self) -> Option<Self::Item> {
        ...
        Some(LinkedNode {
            node,
            parent: Some(self.parent.clone()),
            index: self.index + self.iter.len(),
            offset: self.back,
        })
    }
}
