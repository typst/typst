...
use crate::{SVGRenderer, State, SvgMatrix, SvgRelativePathBuilder};

impl SVGRenderer<'_> {
    /// Render a shape element.
    pub(super) fn render_shape(&mut self, mut state: State, shape: &Shape) {
        ...
        // reset transform because we applied it as element's transform attribute
        // we kept it before for shape paint transform
