...
struct Preparation<'a> {
    ...
    cj_segmenter: LineSegmenter,
    /// The Unicode line break properties for each code point.
    line_break_data: CodePointMapData<LineBreak>,
...
//
// icu4x-datagen --keys-for-bin target/debug/typst --locales full --format blob \
//               --out assets/files/icudata.postcard --overwrite
// The icu4x-datagen is installed by `cargo install icu4x-datagen`.
const ICU_DATA: &'static [u8] = include_bytes!("../../../assets/files/icudata.postcard");
// generated by the following command:
//
// icu4x-datagen --locales zh ja --keys segmenter/line@1 --format blob \
//               --out assets/files/cj_linebreak_data.postcard --overwrite
//
// The icu4x-datagen used here should be patched by
// https://github.com/peng1999/icu4x/commit/b9beb6cbf633d61fc3d7983e5baf7f4449fbfae5
const CJ_LINEBREAK_DATA: &'static [u8] =
    include_bytes!("../../../assets/files/cj_linebreak_data.postcard");
...
fn prepare<'a>(
    ...
    let line_break_data = maps::load_line_break(&deser_provider).unwrap();
...
fn breakpoints<'a>(p: &'a Preparation<'a>) -> Breakpoints<'a> {
    let mut linebreaks = if matches!(p.lang, Some(Lang::CHINESE | Lang::JAPANESE)) {
        p.cj_segmenter.segment_str(p.bidi.text)
    } else {
        p.segmenter.segment_str(p.bidi.text)
    };
...
impl Iterator for Breakpoints<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let lb = self.p.line_break_data.as_borrowed();
