...
impl World for ReportWorld {
    ...
    fn file(&self, id: FileId) -> FileResult<Bytes> {
        ...
        let slot = map.entry(id).or_insert_with(|| FileSlot::new(id));
        slot.file()
    }
...
pub fn image_diff(path_a: &Path, path_b: &Path) -> ImageFileDiff {
    ImageFileDiff {
        left: Image::new(path_a),
        right: Image::new(path_b),
    }
}
