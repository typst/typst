...
impl<'a> Runner<'a> {
    ...
    fn handle_annotations(&mut self) {
        ...
        for Note { status, seen, kind, range, message } in self.test.body.notes.iter() {
            ...
            // Figure out if the diagnostic is only emitted in a specific
            // output/target that isn't hit by all possible branches of the
            // stage tree.
            // E.g. If running Paged and the note was only emitted in Eval, it's
            // still covered because Eval is required everything in Paged.
            ...
            if !fully_covered {
                ...
                let siblings = attr_stages & seen.with_siblings();
                log!(self, "only emitted in [{seen}] but expected in [{siblings}]");
            }

            // Log notes and record if we need to update.
            match &status {
                NoteStatus::Emitted => {
