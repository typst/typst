...
--- array-bad-token paged diagnostic ---
...
--- spread-dict-into-array paged diagnostic ---
...
--- array-range-end-missing paged diagnostic ---
// Error: 2-9 missing argument: end
#range()

--- array-range-float-invalid paged diagnostic ---
// Error: 11-14 expected integer, found float
#range(1, 2.0)

--- array-range-bad-step-type paged diagnostic ---
// Error: 17-22 expected integer, found string
#range(4, step: "one")

--- array-range-step-zero paged diagnostic ---
// Error: 18-19 number must not be zero
#range(10, step: 0)

--- array-bad-method-lvalue paged diagnostic ---
// Test bad lvalue.
// Error: 2:3-2:14 cannot mutate a temporary value
#let array = (1, 2, 3)
#(array.len() = 4)

--- array-unknown-method-lvalue paged diagnostic ---
// Test bad lvalue.
...
--- array-first-empty paged diagnostic ---
// Error: 2-12 array is empty
#().first()

--- array-last-empty paged diagnostic ---
...
--- array-insert-missing-index paged diagnostic ---
...
--- array-fold-closure-without-params paged diagnostic ---
...
--- array-sum-empty paged diagnostic ---
...
--- array-product-empty paged diagnostic ---
...
--- array-join-bad-values paged diagnostic ---
// Error: 2-22 cannot join boolean with boolean
#(true, false).join()

--- array-join-bad-separator paged diagnostic ---
...
--- array-chunks-size-zero paged diagnostic ---
// Error: 19-20 number must be positive
#(1, 2, 3).chunks(0)

--- array-chunks-size-negative paged diagnostic ---
...
--- array-windows-size-zero paged diagnostic ---
// Error: 20-21 number must be positive
#(1, 2, 3).windows(0)

--- array-windows-size-negative paged diagnostic ---
...
--- array-sorted-invalid-by-function paged diagnostic ---
// Error: 2-39 expected boolean from `by` function, got string
#(1, 2, 3).sorted(by: (_, _) => "hmm")

--- array-sorted-key-function-positional-1 paged diagnostic ---
...
--- array-to-dict-bad-item-type paged diagnostic ---
// Error: 2-16 expected (str, any) pairs, found integer
#(1,).to-dict()

--- array-to-dict-bad-pair-length-1 paged diagnostic ---
// Error: 2-19 expected pairs of length 2, found length 1
#((1,),).to-dict()

--- array-to-dict-bad-pair-length-3 paged diagnostic ---
// Error: 2-26 expected pairs of length 2, found length 3
#(("key",1,2),).to-dict()

--- array-to-dict-bad-key-type paged diagnostic ---
// Error: 2-21 expected key of type str, found integer
#((1, 2),).to-dict()

--- array-zip-positional-and-named-argument paged diagnostic ---
// Error: 13-30 unexpected argument: val
#().zip((), val: "applicable")

--- array-sorted-bad-key paged diagnostic ---
// Error: 32-37 cannot divide by zero
#(1, 2, 0, 3).sorted(key: x => 5 / x)

--- array-sorted-uncomparable paged diagnostic ---
// Error: 2-26 cannot compare content and content
#([Hi], [There]).sorted()

--- array-sorted-uncomparable-lengths paged diagnostic ---
// Error: 2-26 cannot compare 3em with 2pt
#(1pt, 2pt, 3em).sorted()

--- array-sorted-key-function-positional-2 paged diagnostic ---
// Error: 42-52 unexpected argument
#((k: "a", v: 2), (k: "b", v: 1)).sorted(it => it.v)

--- issue-3014-mix-array-dictionary paged diagnostic ---
// Error: 8-17 expected expression, found named pair
#(box, fill: red)

--- issue-3154-array-first-empty paged diagnostic ---
#{
  let array = ()
...
--- issue-3154-array-first-mutable-empty paged diagnostic ---
#{
  let array = ()
  // Error: 3-16 array is empty
  array.first() = 9
...
--- issue-3154-array-last-empty paged diagnostic ---
#{
  let array = ()
...
--- issue-3154-array-last-mutable-empty paged diagnostic ---
#{
  let array = ()
  // Error: 3-15 array is empty
  array.last() = 9
...
--- issue-3154-array-at-out-of-bounds paged diagnostic ---
#{
  let array = (1,)
...
--- issue-3154-array-at-out-of-bounds-mutable paged diagnostic ---
#{
  let array = (1,)
...
--- issue-3154-array-at-out-of-bounds-mutable-default paged diagnostic ---
#{
  let array = (1,)
...
--- array-unopened paged diagnostic ---
// Error: 2-3 unclosed delimiter
#{)}

--- array-unclosed paged diagnostic ---
...
--- array-reduce-missing-reducer paged diagnostic ---
// Error: 2-13 missing argument: reducer
#().reduce()

--- array-reduce-unexpected-argument paged diagnostic ---
