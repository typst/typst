...
--- ops-add-too-large paged diagnostic ---
...
--- ops-add-too-large-decimal paged diagnostic ---
// Error: 3-47 value is too large
#(decimal("79228162514264337593543950335") + 1)

--- ops-subtract-too-large-decimal paged diagnostic ---
...
--- ops-not-trailing paged diagnostic ---
...
--- ops-unary-minus-missing-expr paged diagnostic ---
// Error: 4 expected expression
#(-)

--- ops-add-missing-rhs paged diagnostic ---
// Error: 10 expected expression
#test({1+}, 1)

--- ops-mul-missing-rhs paged diagnostic ---
// Error: 10 expected expression
#test({2*}, 2)

--- ops-unary-plus-on-content paged diagnostic ---
// Error: 3-13 cannot apply unary '+' to content
#(+([] + []))

--- ops-unary-plus-on-string paged diagnostic ---
// Error: 3-6 cannot apply '-' to string
#(-"")

--- ops-not-on-array paged diagnostic ---
// Error: 3-9 cannot apply 'not' to array
#(not ())

--- ops-compare-relative-length-and-ratio paged diagnostic ---
// Error: 3-19 cannot compare relative length and ratio
#(30% + 1pt <= 40%)

--- ops-compare-em-with-abs paged diagnostic ---
// Error: 3-14 cannot compare 1em with 10pt
#(1em <= 10pt)

--- ops-compare-normal-float-with-nan paged diagnostic ---
// Error: 3-22 cannot compare 2.2 with float.nan
#(2.2 <= float("nan"))

--- ops-compare-int-and-str paged diagnostic ---
// Error: 3-26 cannot compare integer and string
#((0, 1, 3) > (0, 1, "a"))

--- ops-compare-array-nested-failure paged diagnostic ---
// Error: 3-42 cannot compare 3.5 with float.nan
#((0, "a", 3.5) <= (0, "a", float("nan")))

--- ops-divide-by-zero-float paged diagnostic ---
// Error: 3-12 cannot divide by zero
#(1.2 / 0.0)

--- ops-divide-by-zero-int paged diagnostic ---
// Error: 3-8 cannot divide by zero
#(1 / 0)

--- ops-divide-by-zero-angle paged diagnostic ---
// Error: 3-15 cannot divide by zero
#(15deg / 0deg)

--- ops-binary-arithmetic-error-message paged diagnostic ---
// Special messages for +, -, * and /.
// Error: 3-10 cannot add integer and string
#(1 + "2", 40% - 1)

--- add-assign-int-and-str paged diagnostic ---
// Error: 15-23 cannot add integer and string
#{ let x = 1; x += "2" }

--- ops-divide-ratio-by-length paged diagnostic ---
// Error: 4-13 cannot divide ratio by length
#( 10% / 5pt )

--- ops-divide-em-by-abs paged diagnostic ---
// Error: 3-12 cannot divide these two lengths
#(1em / 5pt)

--- ops-divide-relative-length-by-ratio paged diagnostic ---
// Error: 3-19 cannot divide relative length by ratio
#((10% + 1pt) / 5%)

--- ops-divide-relative-lengths paged diagnostic ---
// Error: 3-28 cannot divide these two relative lengths
#((10% + 1pt) / (20% + 1pt))

--- ops-subtract-int-from-ratio paged diagnostic ---
// Error: 13-20 cannot subtract integer from ratio
#((1234567, 40% - 1))

--- ops-multiply-int-with-bool paged diagnostic ---
// Error: 3-11 cannot multiply integer with boolean
#(2 * true)

--- ops-divide-int-by-length paged diagnostic ---
// Error: 3-11 cannot divide integer by length
#(3 / 12pt)

--- multiply-negative-int-with-str paged diagnostic ---
...
--- ops-assign-unknown-var-lhs paged diagnostic ---
#{
...
--- ops-assign-unknown-var-rhs paged diagnostic ---
#{
  let a = 2
  a = 1-a
  a = a -1
...
--- ops-assign-unknown-parenthesized-variable paged diagnostic ---
// Error: 4-5 unknown variable: x
#((x) = "")

--- ops-assign-destructuring-unknown-variable paged diagnostic ---
// Error: 4-5 unknown variable: x
#((x,) = (1,))

--- ops-assign-to-temporary paged diagnostic ---
// Error: 3-8 cannot mutate a temporary value
#(1 + 2 += 3)

--- ops-assign-to-invalid-unary-op paged diagnostic ---
// Error: 2:3-2:8 cannot apply 'not' to string
#let x = "Hey"
#(not x = "a")

--- ops-assign-to-invalid-binary-op paged diagnostic ---
// Error: 7-8 unknown variable: x
#(1 + x += 3)

--- ops-assign-unknown-variable paged diagnostic ---
// Error: 3-4 unknown variable: z
#(z = 1)

--- ops-assign-to-std-constant paged diagnostic ---
