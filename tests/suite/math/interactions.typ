...
--- math-nested-normal-layout render ---
...
--- math-accent-show-rule-3 render ---
#show math.accent: it => {
...
--- math-accent-show-rule-4 render ---
#show "\u{0302}": box(inset: (bottom: 5pt), text(0.5em, sym.diamond.small))
$hat(X)$, $hat(x)$

--- math-box-without-baseline render ---
...
--- issue-math-realize-hide render ---
$ x^2 #hide[$(>= phi.alt) union y^2 0$] z^2 $
Hello #hide[there $x$]
and #hide[$ f(x) := x^2 $]

--- issue-math-realize-scripting render ---
...
--- math-size-resolve render ---
#let length = context repr(measure("--").width)
$ a length a ^ length $

--- math-size-arbitrary-content render ---
...
--- math-size-math-content-1 render ---
...
--- math-size-math-content-2 render ---
// Nested math content has styles overwritten by the inner equation.
